...
공개 Q&A 페이지는 아래 경로에서만 질문 작성폼과 본인 질문 조회폼을 제공합니다.

- `/qa.html`

홈페이지에는 Q&A 페이지로 이동하는 링크만 노출하고, 질문 작성폼은 메인 화면에 직접 노출하지 않습니다.

비밀글 구조:

- 질문 등록 시 이름 또는 닉네임, 비밀번호, 제목, 질문 내용, 개인정보 동의를 받습니다.
- 질문 비밀번호는 평문 저장하지 않고 salt와 함께 SHA-256 해시로 저장합니다.
- 질문 등록 완료 시 고유 조회 코드가 발급됩니다.
- 공개 목록 API `/api/qa`는 다른 사람 질문을 노출하지 않고 빈 목록만 반환합니다.
- 질문 내용과 답변은 `/api/qa/verify`에서 조회 코드와 비밀번호가 모두 맞을 때만 반환합니다.
- 관리자 답변은 `/admin-secret.html`의 `Q&A 관리` 탭에서 작성합니다.

Q&A APIs:

- `GET /api/qa`: 공개 목록 비노출용 빈 목록 반환
- `POST /api/qa`: 새 질문 등록 및 조회 코드 발급
- `POST /api/qa/verify`: 조회 코드와 비밀번호 검증 후 본인 질문 상세 반환
